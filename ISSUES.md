...
- **Status**: Closed — fixed 2026-08-30 (TODO-33).
...
- **Fix**: Interim on the day of discovery — `'unsafe-eval'` in `script-src`, documented in `index.html` as being there under protest. **Resolved the same day** by TODO-33: `src/mcp/validator.ts` passes the SDK `CfWorkerJsonSchemaValidator`, which interprets the schema instead of generating code, and both `new Client(…)` call sites take it via a shared `CLIENT_OPTIONS`. `script-src` is back to plain `'self'`. Note what did **not** change: AJV is still in the bundle, because `client/index.js` imports it statically — it is simply never instantiated, so the cost is bytes (+21.8 kB raw, +5.8 kB gzipped, since `@cfworker/json-schema` is added rather than substituted), not behaviour.
- **How it is kept fixed**: `validator.test.ts` runs a validation with the global `Function` binding replaced by a throwing proxy. That is a real discriminator, not a ritual — checked both ways before relying on it: the SDK's AJV provider throws under the trap (and prints the function it built from the schema), the interpreting one passes. Verified live afterwards in a real browser against `https://mcp.deepwiki.com/mcp` under the tightened policy: connect, list, and a `read_wiki_structure` call all succeed with zero CSP violations and zero page errors. **The general shape: a policy exception is often a symptom worth reading — the weakened CSP was the cheap problem, and the untrusted-schema-to-code path it exposed was the real one.**
