...
- URL **fragment**: `#server=…` plus at most one of `&tool=`/`&resource=`/`&prompt=` — never headers/tokens. A `server` value connects on arrival only if it is already in this device's recents; anything else waits for the visitor to press Connect (ISSUE-12).
- **Why the fragment and not the query (TODO-31).** A query string is sent to the host in the HTTP request for the document itself, so opening a shared link or reloading handed the address of the visitor's MCP server to GitHub Pages' logs. A fragment is never transmitted and never appears in a `Referer`. The app writes `#` and reads either — `readParams` in `src/ui/selectionUrl.ts` prefers the fragment when it names any of our keys *with a value*, so links shared as `?server=` keep working forever and a result-outline heading slug in the fragment (`#some-heading`, `Outline.tsx`) never shadows them. Verified in a real browser: the document request for a selected page carries no query string at all.

**What leaves the device.** Nothing but MCP traffic to the server the visitor names. There is no backend; `.github/workflows/deploy.yml` uploads a static `dist/` to Pages. The only `fetch` in the codebase is the `no-cors` reachability probe in `src/mcp/probe.ts`, which targets that same address and runs only after a failed connect. No analytics, telemetry, third-party scripts or beacons; fonts are bundled by Fontsource rather than fetched. Headers/tokens live in `localStorage` (`src/ui/recents.ts`) and are attached only to the server URL they were saved against; results are held in memory for the session (`src/ui/run/runHistory.ts`) and never persisted. GitHub Pages of course still sees the request for the page itself — IP and user agent — as any host does. Nothing a server sends is turned into code: the SDK is given an interpreting JSON Schema validator (`src/mcp/validator.ts`) rather than its default, which compiled each `outputSchema` with `new Function` (ISSUE-18 / TODO-33).
- Content-Security-Policy: a `<meta http-equiv>` in `index.html`, because GitHub Pages sends no security headers and cannot be configured to. `script-src 'self'`, with no `'unsafe-eval'`: `src/mcp/validator.ts` hands the SDK an interpreting validator, so no server-supplied `outputSchema` is ever compiled (ISSUE-18 / TODO-33). Also `object-src`/`frame-src`/`form-action` `'none'`, `base-uri 'none'`, `img-src`/`font-src` `'self' data:`, `style-src 'self' 'unsafe-inline'`, and `connect-src *` — the wildcard is the product, since reaching a visitor's own server at any address is what the app does. `frame-ancestors` is absent by necessity (meta tags ignore it, Pages cannot send the header), so clickjacking is unmitigated (ISSUE-14).
