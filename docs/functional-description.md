...
- **Landing screen** — server URL input; an Advanced box for custom headers (e.g. `Authorization: Bearer …`); a recent-servers list (localStorage) with one-click reconnect; a "Try the demo" button running the built-in in-page demo server (**built** — `demo-issue-tracker`: 4 tools, 2 resources, 2 prompts over InMemoryTransport; the polished landing screen itself is not yet built).
